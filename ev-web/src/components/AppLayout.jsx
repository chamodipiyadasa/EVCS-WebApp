    ...
    <div className="min-h-screen flex bg-slate-50">
      <aside className="w-64 bg-slate-900 p-4 space-y-2">
        <div className="text-xl font-bold text-white mb-4">
          EVCS {role === 'Operator' ? 'Operator' : 'Admin'}
        </div>
        ...
        {role === 'Operator' && (
          <>
            <Link to="/app/operator" className={`block px-3 py-2 rounded ${active('/app/operator')}`}>
              Operator Dashboard
            </Link>
            <Link to="/app/operator/bookings" className={`block px-3 py-2 rounded ${active('/app/operator/bookings')}`}>
              Bookings
            </Link>
            <Link to="/app/operator/scan" className={`block px-3 py-2 rounded ${active('/app/operator/scan')}`}>
              Scan QR
            </Link>
          </>
        )}
        ...
        <button className="mt-2 px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 text-white w-full" onClick={logout}>
          Logout
        </button>
      </aside>
